...
function checkEntryAndOut(config){
  ...
  if(keys.indexOf('entry')<0 || keys.indexOf('output')){
    console.log(chalk.bgRed('必须配置entry & output!'));
    flag = false;
  }
  return flag;
...
gulp.task("common-task-babel",(done)=>{
  // TreatWebpackConfig
    try {
      let binsan_tools_path = path.join(cwd, 'biosan-tools.js');
      let biosan_tools_config = require(binsan_tools_path);
    } catch (error) {
      console.log(chalk.bgRed('未发现biosan-tools.js文件在：'+binsan_tools_path));
    }
